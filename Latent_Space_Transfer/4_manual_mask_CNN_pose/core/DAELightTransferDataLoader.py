            ...
            for root, _, fnames in sorted(os.walk(dirpath_root)):
                ...
                for fname in fnames:
                    if is_image_file(fname):
                        ...
                        if session_name in session_maps:
                            if (ids, ide) in session_maps[session_name]:
                                session_maps[session_name][(ids, ide, idp)].append([idl, path_img])
                            else:
                                session_maps[session_name][(ids, ide, idp)] = [[idl, path_img]]
                        else:
                            session_maps[session_name] = {(ids, ide, idp):[[idl, path_img]]}
            ...
            for root, _, fnames in sorted(os.walk(dirpath_root)):
                ...
                for fname in fnames:
                    ...
                    if is_image_file(fname):
                        ...
                        # print(ids, idl)
                        if session_name in session_maps:
                            if (ids, ide, idp) in session_maps[session_name]:
                                session_maps[session_name][(ids, ide, idp)].append([idl, path_img])
                            else:
                                session_maps[session_name][(ids, ide, idp)] = [[idl, path_img]]
                        else:
