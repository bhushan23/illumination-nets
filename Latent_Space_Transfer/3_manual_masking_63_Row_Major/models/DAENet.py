...
class waspGridSpatialIntegral(nn.Module):
    def __init__(self,opt):
        ...
        if self.opt.cuda:
            self.filterx = self.filterx.type(torch.cuda.FloatTensor)
            self.filtery = self.filtery.type(torch.cuda.FloatTensor)
...
class LightingTransfer(nn.Module):
    ...
    def fill_illlumination_map(self):
        illumination_map    = [[]] * 20
        illumination_map[0] = [0] * 3969
        illumination_map[1] = ([0] * 54 + [1] * 9) * 63
        illumination_map[2] = ([0] * 45 + [1] * 18) * 63
        illumination_map[3] = ([0] * 36 + [1] * 27) * 63
        illumination_map[4] = ([0] * 27 + [1] * 36) * 63
        illumination_map[5] = ([0] * 18 + [1] * 45) * 63
        illumination_map[6] = ([0] * 9 + [1] * 54) * 63
        illumination_map[7] = ([0] * 0 + [1] * 63) * 63
        illumination_map[8] = ([1] * 54 + [0] * 9) * 63
        illumination_map[9] = ([1] * 45 + [0] * 18) * 63
        illumination_map[10] = ([1] * 36 + [0] * 27) * 63
        illumination_map[11] = ([1] * 27 + [0] * 36) * 63
        illumination_map[12] = ([1] * 18 + [0] * 45) * 63
        illumination_map[13] = ([1] * 9 + [0] * 54) * 63
        illumination_map[14] = ([0] * 27 + [1] * 36)*9 + ([0] * 18 + [1] * 45) * 45 + ([0] * 27 + [1] * 36)*9
        illumination_map[15] = ([0] * 27 + [1] * 36)*9 + ([0] * 9 + [1] * 54) * 45 + ([0] * 27 + [1] * 36)*9
        illumination_map[16] = ([0] * 9 + [1] * 45 + [0] * 9) * 45 + ([0] * 18 + [1] * 27 + [0] * 18) * 18
        illumination_map[17] = ([1] * 36 + [0] * 27)*9 + ([1] * 45 + [0] * 18) * 45 + ([1] * 36 + [0] * 27)*9
        illumination_map[18] = ([1] * 36 + [0] * 27)*9 + ([1] * 9 + [0] * 54) * 45 + ([1] * 27 + [0] * 36)*9
        illumination_map[19] = [0] * 3969
    ...
    def forward(self, light_direction, encoded_shading):
        ...
        illumination_vector = self.illumination_map[light_direction]
        illumination_vector = torch.tensor(illumination_vector).type(torch.cuda.FloatTensor)
        # print('Illumination Vector' , illumination_vector.shape)
        light_space = self.generate_light_space(illumination_vector)
        # print('Generation Done:', light_space.shape, encoded_shading.shape)
        new_input   = torch.cat((light_space, encoded_shading), 1)
        return self.main(new_input)
...
class Dense_DecodersIntegralWarper2_Intrinsic(nn.Module):
    ...
    def forward(self, lightingDirection, zS, zT, zW, basegrid):
        # print('Lighting Direction Shape', lightingDirection.shape)
