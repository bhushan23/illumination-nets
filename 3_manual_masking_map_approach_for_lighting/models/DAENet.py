...
class LightingTransfer(nn.Module):
    def __init__(self, opt, ngpu=1, nz=49, activation=nn.ReLU, args=[False]):
        super(LightingTransfer, self).__init__()
        self.ngpu = ngpu
    ...
    def fill_illlumination_map(self):
        self.illumination_map[0] = [0]*49
        self.illumination_map[1] = ([0]*6 + [1]*1)*7
        self.illumination_map[2] = ([0] * 5 + [1]*2) * 7
        self.illumination_map[3] = ([0] * 4 + [1] * 3) * 7
        self.illumination_map[4] = ([0] * 3 + [1] * 4) * 7
        self.illumination_map[5] = ([0] *2 + [1] * 5) * 7
        self.illumination_map[6] = ([0] * 1 + [1] * 6) * 7
        self.illumination_map[7] = ([0] * 0 + [1] * 7) * 7
        self.illumination_map[8] = ([1] * 6 + [0]*1) * 7
        self.illumination_map[9] = ([1] * 5 + [0] * 2) * 7
        self.illumination_map[10] = ([1] * 4 + [0] * 3) * 7
        self.illumination_map[11] = ([1] * 3 + [0] * 4) * 7
        self.illumination_map[12] = ([1] * 2 + [0] * 5) * 7
        self.illumination_map[13] = ([1] * 1 + [0] * 6) * 7
        self.illumination_map[14] = [0]*3 + [1]*4 + ([0] * 2 + [1] * 5) * 6 + [0]*3 + [1]*4
        self.illumination_map[15] = [0] * 3 + [1] * 4 + ([0] * 1 + [1] * 6) * 6 + [0] * 3 + [1] * 4
        self.illumination_map[16] = ([0] * 1 + [1] * 5 + [0]*1) * 5 + ([0] * 2 + [1] * 3 + [0]*2) * 2
    ...
    def forward(self, light_direction, encoded_shading):
        ...
        # light_space = self.generate_light_space(light_direction)
        # print('Generation Done:', light_space.shape, encoded_shading.shape)
        new_input   = torch.cat((illumination_vector, encoded_shading), 1)
        return self.main(new_input)
